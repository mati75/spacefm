...
GType ptk_text_renderer_get_type(void)
{
    ...
    if (!cell_text_type)
    {
        ...
        cell_text_type =
            g_type_register_static(GTK_TYPE_CELL_RENDERER, "PtkTextRenderer", &cell_text_info, 0);
    }
...
#define ADD_SET_PROP(propname, propval, nick, blurb) \
    g_object_class_install_property(                 \
        object_class,                                \
        propval,                                     \
        g_param_spec_boolean(propname, nick, blurb, FALSE, G_PARAM_READABLE | G_PARAM_WRITABLE))

    ADD_SET_PROP("background-set",
                 PROP_BACKGROUND_SET,
                 _("Background set"),
                 _("Whether this tag affects the background color"));

    ADD_SET_PROP("foreground-set",
                 PROP_FOREGROUND_SET,
                 _("Foreground set"),
                 _("Whether this tag affects the foreground color"));

    ADD_SET_PROP("underline-set",
                 PROP_UNDERLINE_SET,
                 _("Underline set"),
                 _("Whether this tag affects underlining"));

    ADD_SET_PROP("ellipsize-set",
                 PROP_ELLIPSIZE_SET,
                 _("Ellipsize set"),
...
static void set_bg_color(PtkTextRenderer* celltext, GdkColor* color)
{
    if (color)
    {
...
static void set_fg_color(PtkTextRenderer* celltext, GdkColor* color)
{
    if (color)
    {
...
static void notify_set_changed(GObject* object, PangoFontMask changed_mask)
{
...
static void set_font_description(PtkTextRenderer* celltext, PangoFontDescription* font_desc)
{
                               ...
                               GtkCellRendererState flags)
{
    ...
    if (will_render)
    {
        ...
        if (celltext->foreground_set && (flags & GTK_CELL_RENDERER_SELECTED) == 0)
        {
            PangoColor color;
    ...
    if (cell_area)
    {
        if (x_offset)
        {
            if (gtk_widget_get_direction(widget) == GTK_TEXT_DIR_RTL)
                *x_offset =
                    (1.0 - xalign) * (cell_area->width - (rect.x + rect.width + (2 * xpad)));
            else
