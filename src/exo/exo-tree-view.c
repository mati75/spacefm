...
typedef struct ExoTreeViewPrivate
{
    ...
    unsigned int button_release_unblocks_dnd : 1;

    /* whether rubberbanding must be re-enabled on button-release-event (drag and drop active) */
    unsigned int button_release_enables_rubber_banding : 1;
...
static bool exo_tree_view_button_press_event(GtkWidget* widget, GdkEventButton* event)
{
    ...
    if (G_LIKELY(event->window == gtk_tree_view_get_bin_window(GTK_TREE_VIEW(tree_view))))
    {
        ...
        /* check if the next button-release-event should activate the selected row (single click
         * support) */
        tree_view->priv->button_release_activates =
            (tree_view->priv->single_click && event->type == GDK_BUTTON_PRESS &&
...
static bool exo_tree_view_button_release_event(GtkWidget* widget, GdkEventButton* event)
{
    ...
    if (G_LIKELY(event->window == gtk_tree_view_get_bin_window(GTK_TREE_VIEW(tree_view))))
    {
        /* check if we're in single-click mode and the button-release-event should emit a
         * "row-activate" */
        if (G_UNLIKELY(tree_view->priv->single_click && tree_view->priv->button_release_activates))
        {
                                              ...
                                              NULL))
            {
                ...
                if (!tree_view->priv->activable_column ||
        ...
        {
            /* determine the path on which the button was released and select only this row, this
             * way the user is still able to alter the selection easily if all rows are selected.
             */
            if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(tree_view),
                                              event->x,
                                              event->y,
        ...
        tree_view->priv->single_click)
    {
        ...
        else
        {
                 ...
                 gtk_tree_path_compare(path, tree_view->priv->hover_path) != 0))
            {
                ...
                if (G_LIKELY(path != NULL))
                {
                    /* setup the hand cursor to indicate that the row at the pointer can be
