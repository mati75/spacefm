...
G_DEFINE_TYPE_WITH_CODE(ExoIconChooserModel, exo_icon_chooser_model, G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(GTK_TYPE_TREE_MODEL,
                                              exo_icon_chooser_model_tree_model_init))
...
static void exo_icon_chooser_model_finalize(GObject* object)
{
    ...
    if (G_LIKELY(model->icon_theme != NULL))
    {
        ...
        g_signal_handlers_disconnect_by_func(G_OBJECT(model->icon_theme),
                                             exo_icon_chooser_model_icon_theme_changed,
                                             model);
...
static bool exo_icon_chooser_model_merge_symlinks(void* key, void* value, void* data)
{
    ...
    if (g_path_is_absolute(target) || g_str_has_prefix(target, "../"))
    {
                                                      ...
                                                      ExoIconChooserModel* model)
{
    ...
    symlink_items =
        g_hash_table_new_full(g_str_hash, g_str_equal, NULL, exo_icon_chooser_model_item_free);
    ...
    for (lp = icons; lp != NULL; lp = lp->next)
    {
        ...
        if (G_LIKELY(icon_info != NULL))
        {
            ...
            if (filename != NULL && g_file_test(filename, G_FILE_TEST_IS_SYMLINK))
            {
 ...
 * Since: 0.3.1.9
 **/
ExoIconChooserModel* _exo_icon_chooser_model_get_for_widget(GtkWidget* widget)
{
 ...
 * Since: 0.3.1.9
 **/
ExoIconChooserModel* _exo_icon_chooser_model_get_for_icon_theme(GtkIconTheme* icon_theme)
{
    ExoIconChooserModel* model;
    ...
    if (G_LIKELY(model == NULL))
    {
        ...
        g_signal_connect(G_OBJECT(icon_theme),
                         "changed",
                         G_CALLBACK(exo_icon_chooser_model_icon_theme_changed),
                         model);
    }
    else
    {
